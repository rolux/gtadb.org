...
const map3d6Heights = {
    4: {
        ...
        scale: 0.1,
        zero: [240, 239],
        metersPerValue: 0.006572453230379785,
        elevationOffset: -41.10032245323038,
    },
    5: {
        width: 930,
        height: 1300,
...
const map3d6TileSetRanges = {
    ...
    "cyber,1": {0: [[0, 0], [2, 3]], 1: [[1, 1], [4, 6]], 2: [[2, 3], [9, 12]], 3: [[5, 6], [19, 24]], 4: [[11, 13], [39, 48]], 5: [[22, 26], [79, 97]], 6: [[44, 52], [158, 194]]},
...
function map3d6TextureDefinition(v, tileSet) {
    return map3d6Textures[v] && map3d6Textures[v][tileSet];
}
...
gtadb.Map3D = function(options) {
    ...
    const initialHeight = self.height || map3d6Heights[self.v] || map3d6Heights[6];
    self.height = {...initialHeight, zero: [...initialHeight.zero]};
    ...
    function updateBackground() {
        const definition = map3d6TextureDefinition(self.v, self.tileSet);
    ...
    const terrainProgram = createProgram(`#version 300 es
        ...
        float elevationFromValue(float value) {
            return value * u_meters_per_value + u_elevation_offset;
        }
        ...
        void main() {
            ...
            float centerValue = valueAt(heightUv);
            float leftValue = valueAt(heightUv - vec2(u_texel.x, 0.0));
            float rightValue = valueAt(heightUv + vec2(u_texel.x, 0.0));
            float southValue = valueAt(heightUv - vec2(0.0, u_texel.y));
            float northValue = valueAt(heightUv + vec2(0.0, u_texel.y));
            leftValue = leftValue > 0.0 ? leftValue : centerValue;
            rightValue = rightValue > 0.0 ? rightValue : centerValue;
            southValue = southValue > 0.0 ? southValue : centerValue;
            northValue = northValue > 0.0 ? northValue : centerValue;
            float elevation = elevationFromValue(centerValue);
            float left = elevationFromValue(leftValue);
            float right = elevationFromValue(rightValue);
            float south = elevationFromValue(southValue);
            float north = elevationFromValue(northValue);
    ...
    function sampleElevation(worldX, worldY) {
        if (!self.heightPixels) return null;
        const imageX = height.zero[0] + worldX * height.scale;
        const imageY = height.zero[1] - worldY * height.scale;
        if (
            imageX < 0 || imageX > height.width - 1
            || imageY < 0 || imageY > height.height - 1
        ) return null;
        ...
        return value === 0 ? null : elevationFromValue(value);
    }
    function updateTargetHeight() {
        self.target[1] = sampleElevation(self.target[0], -self.target[2]) || 0;
    }
    ...
    async function loadHeight(version) {
        ...
        const elevations = await self.getElevation();
        const elevation = version === 6 && elevations && elevations.pixels
            ? elevations
            : elevations && (elevations[version] || elevations[`gta${version}`]);
        if (!elevation || !(elevation.pixels instanceof Uint16Array)) {
            throw new Error(`Could not load GTA ${version} elevation data`);
        }
    ...
    function applyHeight(elevation) {
        ...
        updateHeightBounds();
        self.heightPixels = elevation.pixels;
        if (self.heightTexture) gl.deleteTexture(self.heightTexture);
    ...
    function requestTerrainLoad() {
        ...
        const clearLoadingPromise = function() {
            if (self.loadingPromise === promise) self.loadingPromise = null;
        };
        promise.then(clearLoadingPromise, clearLoadingPromise);
        return promise;
    }
    function ensureLoaded() {
        if (self.loaded) return Promise.resolve();
        if (self.loadingPromise) return self.loadingPromise;
        return requestTerrainLoad();
    }
    function invalidateTerrain() {
        self.loaded = false;
        self.loadRevision++;
        self.loadingPromise = null;
        if (self.visible) requestTerrainLoad();
    }
    ...
    that.set = function(values) {
        ...
        if ("tileSet" in values) self.tileSet = values.tileSet;
        if (self.v !== previousVersion || self.tileSet !== previousTileSet) {
            updateBackground();
            invalidateTerrain();
        }
...
gtadb.Map3D.supports = function(v, tileSet) {
    if (v === 6 && ["dupzor,51", "yanis,15"].includes(tileSet)) return false;
    return Boolean(map3d6TextureDefinition(v, tileSet));
};
